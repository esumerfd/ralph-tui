...
**Fuzzy Search**: Just start typing to filter files. Results are ranked by match quality — exact matches first, then partial matches. Press `Esc` to clear the search, or `Backspace` to delete characters.
